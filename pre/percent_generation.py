...
def generate_one(dataset_name, split, features, neg_pos_ratio, load_start, load_end, special_chars, data_root, tokenizer_name, n_jobs, spacy_batch_size, batch_id, percent): 
    ...
    available_mutate = ["sent_replace", "sent_delete", "word_delete", "sent_replace"]
    mutation = "sent_replace"
    # 1. Load data 
    ...
    for sample_id, (_doc, _sum) in enumerate(pairs):
        ...
        for i in range(neg_pos_ratio):
            ...
            if mutation == 'sent_replace':
                ...
                length = len(_sum)
                indices = random.sample(range(length), max(int( percent / 100 * length), 1))
                for idx in indices:
                    sum_mutate[idx] = replace_sentence()
                sum_mutate = " ".join(sum_mutate)
            elif mutation == 'sent_delete':
                length = len(_sum)
                indices = random.sample(range(length), max(int( percent / 100 * length), 1))
                sum_mutate =  ' '.join([_sum[i] for i in range(length) if i not in indices])
            elif mutation == 'word_delete':
                sum_mutate = mutate_delete(" ".join(_sum).split(), percent / 100, string.punctuation)
            else:
                print("???")
                assert(False)
                
            makeup_sums.append(sum_mutate)
        
        line.extend(makeup_sums)
        lines.append(line)

    dumpfile = os.path.join(data_root, dataset_name.split(':')[0], "sentreplace_{}".format(percent), "{}_{}.tsv".format(split, batch_id))
        ...
        for split in cfg.splits:
            for percent in [33, 66]:
                ...
                for batch_id, (load_start, load_end) in enumerate(boundaries):
                    ...
                    generate_one(dataset_name, split, features, cfg.neg_pos_ratio, 
                        load_start, load_end, cfg.special_characters_to_clean, cfg.data_root, 
                        cfg.tokenizer_name, cfg.n_jobs, cfg.spacy_batch_size, batch_id,
                        percent)

                    elapse = time.time() - start_time
                    print ("  Took {:.3f} seconds".format(elapse))
                
                combine_shuffle(["sentreplace"], cfg.data_root, dataset_name, split, str(percent))
