...
class CustomDataset(Dataset):
    def __init__(self, datapath, nums=None, hierarchical = 1):
        self.data = []
        with open(datapath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                for i in range(1, size-1):
                    if hierarchical:
                        if i + hierarchical >= size: break
                        self.data.append([elements[0], elements[i], elements[i+hierarchical]])
                    else:
...
def train_model(model, train_set, max_iter=CFG.MAX_ITERATION, loss_func='CrossEntropyLoss'):
    epochs = CFG.EPOCHS
    optimizer = torch.optim.Adam(model.parameters(), lr = CFG.LR)
    loss_fn = nn.CrossEntropyLoss()
    if loss_func == 'MarginRankingLoss': loss_fn = nn.MarginRankingLoss(margin=1.0)
